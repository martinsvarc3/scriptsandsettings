...
export const formatBulletPoint = (text: string): string => `    • ${text}`;
export const formatNumberedPoint = (num: number, text: string): string => `    ${num}. ${text}`;

// src/utils/scripts.ts
import { Category, Template } from '../types';
import templates from '../data/templates';

export const getScript = (category: Category): Template[] => {
  return templates[category] || [];
};

export const getAllCategories = (): Category[] => {
  return Object.keys(templates) as Category[];
};

export const getScriptByTitle = (category: Category, title: string): Template | undefined => {
  const categoryScripts = templates[category] || [];
  return categoryScripts.find(script => script.title === title);
};

// Beginning of src/data/templates.ts
import { Category, Template } from '../types';
...
'Wholesaling': [
    {
      ...
      fullScript: `
============================================================
HIGH-CONVERTING WHOLESALING PROPERTY ACQUISITION SCRIPT
============================================================
...
'Creative Finance': [
    {
      ...
      fullScript: `
============================================================
CREATIVE FINANCING PROPERTY ACQUISITION SCRIPT
============================================================
...
'Agent Outreach': [
    {
      ...
      fullScript: `
============================================================
REAL ESTATE AGENT PARTNERSHIP DEVELOPMENT SCRIPT
============================================================
...
'Foreclosure': [
    {
      ...
      fullScript: `
============================================================
PRE-FORECLOSURE HOMEOWNER ASSISTANCE SCRIPT
============================================================
...
// src/index.ts
export { Category, Template } from './types';
export { getScript, getAllCategories, getScriptByTitle } from './utils/scripts';
export { formatBulletPoint, formatNumberedPoint } from './utils/formatters';
...
const wholesalingScripts = getScript('Wholesaling');
console.log('Wholesaling scripts:', wholesalingScripts);

// Get a specific script by title
const specificScript = getScriptByTitle('Wholesaling', 'High-Converting Wholesaling Property Acquisition Script');
if (specificScript) {
  console.log('Found script:', specificScript.title);
  console.log('Preview:', specificScript.preview);
  console.log('Full script:', specificScript.fullScript);
}

// Format text using utilities
const bulletPoint = formatBulletPoint('Example bullet point');
const numberedPoint = formatNumberedPoint(1, 'Example numbered point');

// TypeScript usage example
const filterScriptsByCategory = (category: Category): Template[] => {
  return templates[category] || [];
};

// Using the formatted scripts in a React component
const ScriptDisplay: React.FC<{ category: Category }> = ({ category }) => {
